    ...
    B: Backend,
{
    ...
    ///
    /// This has no effect when the viewport is not [`Viewport::Inline`].
    ///
    /// # Example
    ///
    /// ```rust,ignore
    ...
    pub fn set_viewport_height(&mut self, new_height: u16) -> Result<(), B::Error> {
        ...
        let old_height = core::mem::replace(height, new_height);
        self.clear()?;

        let new_y = match new_height.cmp(&old_height) {
            core::cmp::Ordering::Greater => {
                let overflow = (self.viewport_area.y + new_height)
