...
RUN find /android-ndk -name "*clang" -exec sh -c 'for c; do case "$c" in *x86_64*) "$c" -shared -o /android-ndk/sysroot/usr/lib/x86_64-linux-android/libreadline.so -x c /dev/null 2>/dev/null && exit 0 ;; esac; done' _ {} +
