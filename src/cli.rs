        ...
        match FlylineArgs::try_parse_from(args_with_prog) {
            Ok(parsed) if !args.is_empty() => {
                ...
                match parsed.command {
                    ...
                    Some(Commands::Settings { all, commands }) => {
                        show_settings(&self.settings, all, commands);
                    }
                }
...
fn show_settings(settings: &settings::Settings, all: bool, commands: bool) {
    let diff = settings.diff();

    if commands {
        let changed_commands: Vec<_> = diff
            .iter()
            .filter(|e| !e.is_default)
            .filter_map(|e| e.cli_command.as_deref())
            .collect();
        ...
        println!("# Flyline configuration");
        println!("# Generated by: flyline settings --commands\n");
    ...
    let has_non_defaults = diff.iter().any(|e| !e.is_default);

    if !all && !has_non_defaults {
        println!("All flyline settings are at their default values.");
    ...
    for category in categories {
        let cat_entries: Vec<_> = diff
            .iter()
            .filter(|e| e.category == category && (all || !e.is_default))
            .collect();
    ...
    println!(
        "\nTip: Run 'flyline settings --commands' to generate reproducible ~/.bashrc commands."
    );
}
...
mod tests {
    ...
    fn test_flyline_settings_command_parse() {
        ...
        let args_cmds = FlylineArgs::try_parse_from(["flyline", "settings", "--commands"]).unwrap();
        assert!(matches!(
            args_cmds.command,
            Some(Commands::Settings {
                all: false,
